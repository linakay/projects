...
def cdf_norm(pixels):
    ...
    norm = normalize(cdf)
    normalized = [round(max(pixels)*x) for x in norm]
...
def hgram(bins, labels, fname):
    bars = [f"{round(x*70/max(bins))*'█'}"+f'({x:,})' for x in bins]
...
# Finally let's write the enhanced image to a file
def writepgm(pixels, x, y, filename):
    pixs = [str(p) for p in pixels]
    newpix = '\n'.join(pixs)
    header = f'P2\n{x} {y}\n{max(pixels)}\n'
    filename = f'/tmp/{filename}_new.pgm'
    # filename += '_new.pgm'    # uncomment to save for viewing - the pictures look nice :)
    with open(filename, 'w') as newpic:
        newpic.write(header+newpix)
        newpic.close()
...
try:
    ...
    pixel_counts = count_pixels(pixels)
    bins = make_bins(pixel_counts, mn, mx)[1]
    labels = make_bins(pixel_counts, mn, mx)[0]
    if len(set(pixel_counts)) < 10 or mean(bins)>(5*stdev(bins)):
        new_pixels = sqrt_contrast(pixels)
    else:
        new_pixels = cdf_norm(pixels)
    new_mn, new_mx = min(new_pixels), max(new_pixels)
    new_bins = make_bins(count_pixels(new_pixels), new_mn, new_mx)[1]
    new_labels  = make_bins(count_pixels(new_pixels), new_mn, new_mx)[0]
    hgram(bins, labels, f'Pixel Intensity Values: {fname}')
    hgram(new_bins, new_labels, f'Modified {fname}')
    writepgm(new_pixels, parts[0], parts[1], fname)
